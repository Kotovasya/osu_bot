...
namespace osu_bot.Bot.Parsers
{
    public class PlaysParser : Parser
    {
        ...
        {
            UserScoreQueryParameters parameters = new(ScoreType.Recent, false)
            {
                Limit = 100
            };
            ...
            {
                parameters.UserId = user.OsuUser.Id;
                IEnumerable<OsuScore>? lastScores = await _service.GetUserScoresAsync(parameters);
                if (lastScores is null)
                    throw new NotImplementedException();

                lastScores = lastScores.Where(s => !s.Beatmap.IsScoreable);
                ...
                foreach (OsuScore lastScore in lastScores)
                {
                    ...
                    if (findScore is not null && lastScore.Score > findScore.Score)
                    {
